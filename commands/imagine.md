...
Default model is `gemini-2.5-flash-image` (free-tier quota). `--model gemini-3-pro-image-preview` and Imagen models need a paid tier — if the API returns a quota/permission error, say so plainly. Report the written file path back to the user.
